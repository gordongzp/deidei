...
class Tree {
	...
	* @param array 2维数组，例如：
	* array(
	*      1 => array($this->master_id=>'1','parentid'=>0,'name'=>'一级栏目一'),
	*      2 => array($this->master_id=>'2','parentid'=>0,'name'=>'一级栏目二'),
	*      3 => array($this->master_id=>'3','parentid'=>1,'name'=>'二级栏目一'),
	*      4 => array($this->master_id=>'4','parentid'=>1,'name'=>'二级栏目二'),
	*      5 => array($this->master_id=>'5','parentid'=>2,'name'=>'二级栏目三'),
	*      6 => array($this->master_id=>'6','parentid'=>3,'name'=>'三级栏目一'),
	...
	*/
	...
	public function get_array($myid=0){
		$child = $this->get_child($myid);
		if(is_array($child)){
			foreach($child as $id=>$value){
				$new_child = $this->get_array($id,$value);
				$child[$id]['sub'] = $new_child;
			}
		}
		return $child;
	}
	...
	*/
	...
	public function creat_sub_json($myid, $str='') {
		...
		if(is_array($sub_cats)) foreach($sub_cats as $c) {
			$data[$n][$this->master_id] = iconv(CHARSET,'utf-8',$c['catid']);
