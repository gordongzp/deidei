...
<body style="background-color: #FFF; overflow: auto;">
	...
	<div class="page">
		...
		<form id="add_form" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" name="scence_id" value="<?php echo ($scence_id); ?>" />
				...
				<dl class="row">
					...
					<dd class="opt" id="divComUploadContainer">
						...
						<div id="thumbnails" class="ncap-thumb-list">
							...
							<ul>
								<?php if(($attachment) != ""): if(is_array($attachment)): $i = 0; $__LIST__ = $attachment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$file): $mod = ($i % 2 );++$i;?><li id="file_<?php echo ($file["scence_id"]); ?>_<?php echo ($file["atta_id"]); ?>">
											...
											<a title="删除" class="del" href="javascript:del_file_upload(<?php echo ($file["atta_id"]); ?>,'file_<?php echo ($file["scence_id"]); ?>_<?php echo ($file["atta_id"]); ?>','<?php echo ($file["path"]); ?>');">X</a>
	...
	$(document).ready(function(){
		...
		$('input[name="fileupload"]').each(function(){
			$(this).fileupload({
				dataType: 'json',
				url: '<?php echo U("scence/upload_attachment");?>',
	...
	function del_file_upload(aid,file_id,file_path){
		...
		var nid = '<?php echo ($scence_id); ?>';
		$.getJSON('<?php echo U("scence/remove_attachment");?>', {'aid':aid,'nid':nid,'file_path':file_path},function(json){
			if(json.status==1){
				$('#' + file_id).remove();
			}else{
				alert('删除失败');
