...
<body style="background-color: #FFF; overflow: auto;">
	...
	<script>
		$(function(){
			...
			$('span[nc_type="inline_edit"]').inline_edit({c:'NewsCategory',a:'ajax_save_data'});
		});
		
		function fg_operation(name, bDiv) {
			if (name == 'add') {
				window.location.href = '<?php echo U("NewsCategory/add");?>';
			}else if (name == 'del') {
		...
		function fg_del(ids){
			...
			if(confirm('确认删除此菜单及其所有子菜单吗？')){
				$.getJSON('<?php echo U("NewsCategory/del");?>', {id:id}, function(data){
					if (data.status) {
						location.reload();
					} else {
						showError(data.info)
