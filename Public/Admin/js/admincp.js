...
function openItem(param) {
    ...
    $('li[data-param="menu' + data_str[2] + '"]').addClass('active');
    $this.parent().addClass('active').parents('dl:first').addClass('active').parents('div:first').show();
    //$('#workspace').attr('src', ADMIN_SITE_URL + '/' + data_str[0] + '/' + data_str[1]);
	$('#workspace').attr('src',  '/index.php?c=' + data_str[0] + '&a=' + data_str[1] + '&menu_id=' + data_str[3]);
	$.cookie('workspaceParam', data_str[0] + '|' + data_str[1] + '|' + data_str[2] + '|' + data_str[3], { expires: 1 ,path:"/"});
}